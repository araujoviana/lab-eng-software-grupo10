...
\section{Wireframes (Protótipos de Interface)}
...
\textbf{Objetivo:} classificar a demanda e definir o encaminhamento adequado. \\
\textbf{Pré-condições:} ticket existente com status \texttt{Aberto} ou \texttt{Em triagem}. \\
\textbf{Pós-condições (sucesso):} ticket classificado com prioridade e decisão de triagem registrada (atribuição de responsável para execução, encerramento ou cancelamento). \\
...
\begin{itemize}
    ...
    \item B4 -- Ticket resolvido sem execução: triagem encerra ticket com justificativa e registra auditoria.
\end{itemize}
...
\begin{itemize}
    ...
    \item RN-07: encerramento administrativo exige motivo de encerramento registrado.
\end{itemize}
...
\subsection{Ferramentas e tecnologias adotadas}

\begin{itemize}
    \item Frontend em \textbf{Vue.js} para a interface web do sistema;
    \item Backend em \textbf{Spring Boot (Java)} para API e regras de negócio;
    \item Banco de dados relacional \textbf{PostgreSQL} para persistência principal;
    \item Armazenamento de anexos e evidências em \textbf{Amazon S3};
    \item Logs e monitoramento inicial em \textbf{CloudWatch Logs};
    \item Versionamento e CI/CD com \textbf{GitHub} e \textbf{GitHub Actions};
    \item Modelagem técnica com \textbf{PlantUML} e \textbf{draw.io}.
\end{itemize}
